                ...
                return (
                    <g key={t}>
                        <line x1={padL} y1={y} x2={padL + innerW} y2={y}
                            stroke={t === 0 ? "#94a3b8" : "#e2e8f0"} strokeWidth={t === 0 ? 1.2 : 0.8} strokeDasharray={t === 0 ? "none" : "3,3"} />
                        <text x={padL - 4} y={y + 3.5} textAnchor="end" fontSize="9" fill="#64748b">{t}</text>
                    </g>
                );
            })}

            {/* Danger zone */}
            <rect x={padL} y={padT} width={innerW} height={(innerH * 30) / 100}
                fill="#fef2f2" opacity="0.6" />
            <text x={padL + innerW - 2} y={padT + (innerH * 30) / 100 - 2}
                textAnchor="end" fontSize="8" fill="#dc2626" opacity="0.8">위험 구간 (70%+)</text>

            <path d={areaPath} fill="url(#chartGrad)" />
            <polyline fill="none" stroke="#1d4ed8" strokeWidth="2" points={polyline} strokeLinejoin="round" />
...
function MiniBar({ value, max = 100, color = "#1d4ed8", bgColor = "#e2e8f0" }) {
    ...
    return (
        <div style={{ display: "flex", alignItems: "center", gap: 6 }}>
            <div style={{ flex: 1, height: 7, background: bgColor, borderRadius: 4, overflow: "hidden" }}>
                <div style={{ width: `${pct}%`, height: "100%", background: color, borderRadius: 4 }} />
            </div>
            <span style={{ fontSize: 10, color: "#374151", fontWeight: 700, minWidth: 32, textAlign: "right" }}>
                {value.toFixed(1)}%
            </span>
        </div>
    );
...
export default function PrintableReport({ analysisData, inlineFrameStats, publicItems, reportDate }) {
    ...
    const sortedTop4 = [...analysisData.timeline_chart]
        .sort((a, b) => b.fake_prob - a.fake_prob)
        .slice(0, 4)
        .map(d => d.frame_idx);

    const avgProb = analysisData.timeline_chart.length
        ? analysisData.timeline_chart.reduce((s, d) => s + d.fake_prob, 0) / analysisData.timeline_chart.length
        : 0;
    ...
    const S = {
        ...
        brandBar: {
            ...
            borderBottom: "3px solid #1e3a8a", paddingBottom: 10, marginBottom: 14,
        },
        ...
        infoCell: {
            padding: "6px 10px", borderRight: "1px solid #e2e8f0",
        },
        infoCellLast: { padding: "6px 10px" },
        infoLabel: { fontSize: 9, color: "#64748b", fontWeight: 700, textTransform: "uppercase", letterSpacing: 0.4, marginBottom: 2 },
        ...
        // Section title (InBody style)
        sectionTitle: {
            fontSize: 13, fontWeight: 900, color: "#1e3a8a",
            borderBottom: "1.5px solid #1e3a8a", paddingBottom: 3, marginBottom: 8,
        ...
        metricCard: {
            border: "1px solid #e2e8f0", borderRadius: 5, padding: "8px 10px",
        ...
        // Score card (right panel)
        scoreCard: {
            border: "1.5px solid #1e3a8a", borderRadius: 6,
        ...
        scoreNote: { fontSize: 9, color: "#64748b", marginTop: 6, lineHeight: 1.5 },

        // Analysis table
        table: { width: "100%", borderCollapse: "collapse", fontSize: 11, marginBottom: 10 },
        th: { background: "#f1f5f9", border: "1px solid #e2e8f0", padding: "6px 8px", textAlign: "left", fontWeight: 700, color: "#374151" },
        td: { border: "1px solid #e2e8f0", padding: "6px 8px", verticalAlign: "middle", color: "#374151" },

        // Frame list
        frameGrid: { display: "grid", gridTemplateColumns: "repeat(2, 1fr)", gap: 5, marginTop: 8 },
        frameCard: { border: "1px solid #e2e8f0", borderRadius: 5, padding: "6px 9px", background: "#f8fafc" },
        frameCardTop: { border: "1px solid #fed7aa", borderRadius: 5, padding: "6px 9px", background: "#fff7ed" },
        ...
        hmEmpty: { width: "100%", height: 150, background: "#f1f5f9", display: "flex", alignItems: "center", justifyContent: "center", color: "#94a3b8", fontSize: 11, fontWeight: 700 },
        hmMeta: { padding: "6px 8px", fontSize: 10, lineHeight: 1.6, color: "#374151" },

        // Footer
        footer: { borderTop: "1px solid #e2e8f0", paddingTop: 8, display: "flex", justifyContent: "space-between", fontSize: 9, color: "#94a3b8", marginTop: 10 },

        // Right panel items
        rightItem: { border: "1px solid #e2e8f0", borderRadius: 5, padding: "8px 10px", marginBottom: 6, background: "#f8fafc" },
        rightLabel: { fontSize: 9, color: "#64748b", fontWeight: 700, marginBottom: 2 },
        rightValue: { fontSize: 13, fontWeight: 900, color: "#1e293b" },

        // Control box (체중조절 equivalent)
        controlBox: {
            border: "1px solid #e2e8f0", borderRadius: 5, padding: "8px 10px",
        ...
        <div style={{ background: "#f1f5f9", padding: 20 }}>
            ...
            * { font-family: 'Noto Sans KR', sans-serif !important; }
            ...
            <div style={S.page} className="pdf-page">
                ...
                <div style={S.brandBar}>
                    <div style={S.brandLeft}>
                        <div style={{ display: "flex", alignItems: "baseline", gap: 10 }}>
                            ...
                            <span style={{ fontSize: 10, color: "#94a3b8" }}>[{analysisData.analysis_id}][VideoScope]</span>
                        </div>
                ...
                <div style={{ ...S.infoStrip, gridTemplateColumns: "repeat(4, 1fr)", marginBottom: 14 }}>
                    {[
                        ...
                        { label: "총 프레임 수", value: `${analysisData.timeline_chart.length}프레임` },
                ...
                <div style={S.mainGrid}>
                    <div>
                        ...
                        <div style={{ marginBottom: 12 }}>
                            ...
                            <div style={S.verdictBox}>
                                <div>
                                    <div style={{ fontSize: 10, color: verdictColor, fontWeight: 700, marginBottom: 2 }}>최종 판정</div>
                        ...
                        <div style={{ marginBottom: 12 }}>
                            ...
                            {/* Bar chart per frame */}
                            <div style={{ border: "1px solid #e2e8f0", borderRadius: 5, padding: "10px 12px", background: "#f8fafc", marginBottom: 8 }}>
                                        ...
                                        {modelNames.map((name, i) => {
                                            ...
                                            return (
                                                <tr key={i}>
                                                    ...
                                                    <td style={S.td}>
                                                        <MiniBar value={Math.max(0, Math.min(prob, 100))}
                    ...
                    <div>
                        ...
                        <div style={S.controlBox}>
                            ...
                            {[
                                ...
                                <div key={i} style={S.controlRow}>
                                    ...
                                    <span style={{ ...S.controlValue, color: r.value.includes("강력") || r.value.includes("즉시") ? "#dc2626" : "#1e293b" }}>{r.value}</span>
                                </div>
                            ))}
                        </div>
                        ...
                        <div style={S.controlBox}>
                            ...
                            {[
                                { label: "최고 위조확률", value: `${Math.max(...analysisData.timeline_chart.map(d => d.fake_prob)).toFixed(1)}%` },
                                { label: "최저 위조확률", value: `${Math.min(...analysisData.timeline_chart.map(d => d.fake_prob)).toFixed(1)}%` },
                                { label: "평균 위조확률", value: `${avgProb.toFixed(1)}%` },
                                { label: "위험 프레임 비율", value: `${((inlineFrameStats.dangerCount / analysisData.timeline_chart.length) * 100).toFixed(1)}%` },
                        ...
                        <div style={{
                            ...
                            padding: "8px 10px", background: "#eff6ff", textAlign: "center"
                        }}>
                            <div style={{ fontSize: 9, color: "#1d4ed8", fontWeight: 700, marginBottom: 3 }}>파일 형식</div>
                            <div style={{ fontSize: 20, fontWeight: 900, color: "#1e3a8a" }}>.{fileExt.toUpperCase()}</div>
                            <div style={{ marginTop: 6, display: "flex", flexWrap: "wrap", gap: 4, justifyContent: "center" }}>
                                {modelNames.map(n => (
                                    <span key={n} style={{
                                        padding: "2px 7px", borderRadius: 999,
                                        background: "#dbeafe", border: "1px solid #93c5fd",
                                        color: "#1d4ed8", fontSize: 9, fontWeight: 800
                                    }}>{n}</span>
                                ))}
                            </div>
                        </div>
                ...
                <div style={{ marginBottom: 12 }}>
                    <div style={S.sectionTitle}>
                        비만분석 <span style={S.sectionEn}>Obesity Analysis — Frame Risk Distribution</span>
                    </div>
                    <div style={S.frameGrid}>
                        {analysisData.timeline_chart.map((frame) => {
                            const isTop = sortedTop4.includes(frame.frame_idx);
                            const riskColor = frame.fake_prob >= 70 ? "#dc2626" : frame.fake_prob >= 50 ? "#d97706" : "#16a34a";
                            return (
                                <div key={frame.frame_idx} style={isTop ? S.frameCardTop : S.frameCard}>
                                    <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 3 }}>
                                        ...
                                        <span style={{ fontSize: 14, fontWeight: 900, color: riskColor }}>
                                            {frame.fake_prob.toFixed(1)}%
                                        </span>
                                    </div>
                                    <MiniBar value={frame.fake_prob}
                                        color={frame.fake_prob >= 70 ? "#dc2626" : frame.fake_prob >= 50 ? "#f59e0b" : "#1d4ed8"} />
            ...
            <div style={{ ...S.page, marginTop: 0 }} className="pdf-page">
                        ...
                        <tbody>
                            {publicItems.map((item, idx) => (
                                <tr key={idx} style={{ background: idx % 2 === 0 ? "#fff" : "#f8fafc" }}>
                                    ...
                                    <td style={S.td}>
                                        <MiniBar value={item.score_percent}
                ...
                {/* 신체변화 equivalent — 영상 이력 / 분석 신뢰도 timeline */}
                <div style={{ marginBottom: 14 }}>
                    <div style={S.sectionTitle}>
                        신체변화 <span style={S.sectionEn}>Body Composition History — Analysis Confidence Timeline</span>
                    </div>
                    <div style={{
                        border: "1px solid #e2e8f0", borderRadius: 5, padding: "10px 14px",
                        background: "#f8fafc", overflowX: "auto"
                    }}>
                        {/* Mini table showing frame progression */}
                        <table style={{ ...S.table, marginBottom: 0, fontSize: 10 }}>
                            <thead>
                                <tr>
                                    <th style={{ ...S.th, fontSize: 9 }}>항목 \ 프레임</th>
                                    {analysisData.timeline_chart.map(d => (
                                        <th key={d.frame_idx} style={{ ...S.th, textAlign: "center", fontSize: 9, padding: "4px 5px" }}>
                                            {d.frame_idx}
                                        </th>
                                    ))}
                                </tr>
                            ...
                            <tbody>
                                <tr>
                                    ...
                                    {analysisData.timeline_chart.map(d => (
                                        <td key={d.frame_idx} style={{
                                            ...S.td, textAlign: "center", fontSize: 9, padding: "4px 5px",
                                            fontWeight: 800,
                                            color: d.fake_prob >= 70 ? "#dc2626" : d.fake_prob >= 50 ? "#d97706" : "#16a34a"
                                        }}>
                                ...
                                <tr>
                                    ...
                                    {analysisData.timeline_chart.map(d => (
                                        <td key={d.frame_idx} style={{ ...S.td, textAlign: "center", fontSize: 9, padding: "4px 5px" }}>
                    ...
                    <div style={S.hmGrid}>
                        {analysisData.heatmap_frames.slice(0, 4).map((frame, idx) => (
                            <div key={idx} style={S.hmCard}>
                                {frame.image
                                    ? <img src={frame.image} alt={`heatmap-${frame.id}`} style={S.hmImg} crossOrigin="anonymous" />
                                    : <div style={S.hmEmpty}>히트맵 이미지 없음</div>
                                }
                                <div style={S.hmMeta}>
                                    <div style={{ fontWeight: 800, color: "#1e293b", marginBottom: 1 }}>{frame.id}</div>
                ...
                <div style={{ display: "flex", alignItems: "center", gap: 8, marginBottom: 10 }}>
                    ...
                    {modelNames.map(n => (
                        <span key={n} style={{
                            ...
                            color: "#3730a3", fontSize: 10, fontWeight: 800
                        }}>{n}</span>
                    ))}
                </div>
